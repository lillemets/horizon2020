...
subtitle: "An applicaton of some text mining methods"
...
All words were stemmed, so that only stems of words and not the original words are shown. Compounds were generated from two or more words that often occurred together, here separated by an underscore (*_*).
...
Only 200 most unique terms (in terms of document mean TF-IDF).
...
This is an algorithm that attempts to determine topics by defining a combination of words that would characterize each topic. 
...
We can observe how well model predicted topics for each activity coincides with calls related to each topic:
...
This section is not so relevant for investigating topics.
...
Flesch's Reading Ease Score (Flesch 1948)^[Flesch, R. (1948). A New Readability Yardstick. Journal of Applied Psychology, 32(3), 221.]. Based on number of words, syllabiles and average sentence length. Higher score indicatest that a given text is easier to read.
